...
export class FactoryDAO {
    ...
    static async findFactoryByUserId(userId: string) {
        ...
        return {
            ...
            builder_pad_building_timers: Array.from({ length: 3 }, (_, i) => pads.find(p => p.padIndex === i)?.timer?.toISOString() || ""),
            builder_pad_items_being_built: Array.from({ length: 3 }, (_, i) => pads.find(p => p.padIndex === i)?.itemBeingBuilt || ""),
            builder_pad_items_being_built_secondary: Array.from({ length: 3 }, (_, i) => pads.find(p => p.padIndex === i)?.secondaryItemIndex ?? -1),
            rockets: res.rockets,
