...
import { energyGenerators } from "../models/schema.js";
    ...
    static async createEnergyGenerator(energyGeneratorData: any) {
        try {
            await db.insert(energyGenerators).values({
                ...
                upgradeTimer: energyGeneratorData.upgrade_timer ? new Date(energyGeneratorData.upgrade_timer) : null,
            }).onConflictDoUpdate({
    ...
    static async saveEnergyGeneratorProfile(
        ...
        try {
            await db.insert(energyGenerators).values({
                ...
                upgradeTimer: energyGeneratorProfile.upgrade_timer ? new Date(energyGeneratorProfile.upgrade_timer) : null,
            }).onConflictDoUpdate({
                target: energyGenerators.userId,
                set: {
                    panelCount: energyGeneratorProfile.panel_count,
