...
import { factories, factorySpaceships, builderPads } from "../models/schema.js";
...
export default class FactoryDAO {
        ...
        try {
            await db.transaction(async (tx) => {
                await tx.insert(factories).values({
                    ...
                    factoryUpgradeTimer: factoryInfo.factory_upgrade_timer ? new Date(factoryInfo.factory_upgrade_timer) : null,
                ...
                await tx.delete(factorySpaceships).where(eq(factorySpaceships.userId, factoryInfo.userId));
                if (factoryInfo.spaceships?.length > 0) {
                    await tx.insert(factorySpaceships).values(factoryInfo.spaceships.map((count: number, index: number) => ({
                        userId: factoryInfo.userId,
                        spaceshipType: index,
                        count
                    })));
                ...
                await tx.delete(builderPads).where(eq(builderPads.userId, factoryInfo.userId));
                if (factoryInfo.builder_pad_building_timers?.length > 0) {
                    await tx.insert(builderPads).values(factoryInfo.builder_pad_building_timers.map((timer: string, index: number) => ({
                        userId: factoryInfo.userId,
                        padIndex: index,
                        timer: timer ? new Date(timer) : null,
                        itemBeingBuilt: factoryInfo.builder_pad_items_being_built[index],
                        secondaryItemIndex: factoryInfo.builder_pad_items_being_built_secondary[index]
                    })));
                }
            });
            return factoryInfo;
        } catch (error) {
            logger.error(`[FactoryDAO.createFactory] Error for userId: ${factoryInfo.userId}`, { error });
    ...
    static async findFactoryByUserId(userId: string) {
        try {
            ...
            logger.error(`[FactoryDAO.findFactoryByUserId] Error for userId: ${userId}`, { error });
            throw ERRORS.DB_ERROR(
        ...
        try {
            await db.transaction(async (tx) => {
                await tx.insert(factories).values({
                    ...
                    factoryUpgradeTimer: factoryProfile.factory_upgrade_timer ? new Date(factoryProfile.factory_upgrade_timer) : null,
                ...
                await tx.delete(factorySpaceships).where(eq(factorySpaceships.userId, factoryProfile.userId));
                if (factoryProfile.spaceships?.length > 0) {
                    await tx.insert(factorySpaceships).values(factoryProfile.spaceships.map((count: number, index: number) => ({
                        userId: factoryProfile.userId,
                        spaceshipType: index,
                        count
                    })));
                ...
                await tx.delete(builderPads).where(eq(builderPads.userId, factoryProfile.userId));
                if (factoryProfile.builder_pad_building_timers?.length > 0) {
                    await tx.insert(builderPads).values(factoryProfile.builder_pad_building_timers.map((timer: string, index: number) => ({
                        userId: factoryProfile.userId,
                        padIndex: index,
                        timer: timer ? new Date(timer) : null,
                        itemBeingBuilt: factoryProfile.builder_pad_items_being_built[index],
                        secondaryItemIndex: factoryProfile.builder_pad_items_being_built_secondary[index]
                    })));
                }
            });
            return factoryProfile;
        } catch (error) {
            logger.error(`[FactoryDAO.saveFactoryProfile] Error for userId: ${factoryProfile.userId}`, { error });
            throw ERRORS.DB_ERROR(
