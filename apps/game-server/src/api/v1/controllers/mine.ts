...
export class mineController {
    static async getMineProfile(req: Request, res: Response, next: NextFunction) {
        try {
            const userId = req.params.userId;
            if (_.isArray(userId)) throw ERRORS.VALIDATION("Invalid userId");
            const mineProfile = await MineDAO.findMineByUserId(userId as string);
    ...
    static async upgrade(req: Request<any, any, MineUpgradeInput>, res: Response, next: NextFunction) {
        try {
            ...
            if (operation === "start") {
                const startToUpgradeTime = await MineService.upgradeMinerStart(userId, mineId);
