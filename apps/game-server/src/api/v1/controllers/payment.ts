...
import DonationService from "@/services/payment/DonationService.js";
...
export default class PaymentController {
    static async processDonationCallback(
            ...
            if (!payload)
                return next(
                    INVALID_PAYMENT_CALLBACK(
                        new Error("Missing webhook payload")
                    )
                );
            await DonationService.handleDonationCallback(payload);
