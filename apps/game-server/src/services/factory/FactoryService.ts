...
export default class FactoryService {
    static async upgradeFactoryStart(userId: string): Promise<string> {
        try {
            ...
            if (
                factoryProfile.builder_pad_building_timers.some(t => t !== "")
            ) {
    ...
    static async upgradeFactoryEnd(userId: string, skipWithGem: boolean) {
        try {
            ...
            if (skipWithGem) {
                ...
                const gemsToBePaid = turnTimeInMsToGemsToBePaid(
    ...
    static async buildItemStart(
        ...
        try {
            ...
            if (
                padId === 0 &&
                currentLevel < FACTORY_BUILDING_PADS.pad1.minLevel
            )
                throw ERRORS.VALIDATION("Invalid padId");
            if (
                padId === 1 &&
                currentLevel < FACTORY_BUILDING_PADS.pad2.minLevel
            )
                throw ERRORS.VALIDATION("Invalid padId");
            if (
                padId === 2 &&
                currentLevel < FACTORY_BUILDING_PADS.pad3.minLevel
            )
                throw ERRORS.VALIDATION("Invalid padId");
            ...
            if (
                ...
                itemId === "spaceship" &&
                factoryProfile.spaceships.filter(i => i !== -1).length >=
                ...
                itemId === "cyborg" &&
                factoryProfile.cyborg >= FACTORY_ITEMS_COST_INFO.cyborg.maxCount
            )
                throw ERRORS.VALIDATION("Max count reached");
            else if (
                itemId === "dysonSphere" &&
            ...
            await ProfileService.deductMineralAndCoin(
                userId,
                FACTORY_ITEMS_COST_INFO[itemId].mineralCost,
                FACTORY_ITEMS_COST_INFO[itemId].coinCost
            );
    ...
    static async buildItemEnd(
        ...
        try {
            ...
            if (!FACTORY_BUILT_ITEM_IDS.includes(itemId))
                throw ERRORS.VALIDATION("Invalid itemId");
            if (factoryProfile.builder_pad_items_being_built[padId] !== itemId)
                throw ERRORS.VALIDATION("Invalid itemId");
            ...
            if (skipWithGem) {
                if (passedTime >= timeToWait)
                    throw ERRORS.VALIDATION("Already ended");
                await ProfileService.deductGems(
                    userId,
                    turnTimeInMsToGemsToBePaid(timeToWait - passedTime)
                );
            } else {
                if (passedTime < timeToWait)
                    throw ERRORS.VALIDATION("Not enough time passed");
