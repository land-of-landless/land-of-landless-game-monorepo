...
export default class EnergyGeneratorService {
    static async upgradeEnergyGeneratorStart(userId: string) {
        try {
            ...
            let newLevel = (energyGeneratorProfile.level + 1) as EnergyGeneratorLevelsType;
            let coinsToBePaid = ENERGY_GENERATOR_UPGRADE_INFO[newLevel].coinCost;
    ...
    static async addPanel(userId: string) {
        try {
            ...
            if ((ENERGY_GENERATOR_UPGRADE_INFO as any)[currentLevel].maxPanels < newPanelCount) throw ERRORS.VALIDATION("Max panel count reached");
